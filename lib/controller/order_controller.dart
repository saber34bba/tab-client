...
class OrderController extends GetxController implements GetxService {
  ...
  Future<LatLng> getDeliveryManLocation(String orderID) async {
    var response;
    try {
      response = await orderRepo.getDeliveryManData(orderID);
    } catch (e) {}
    if(response == null || response.body == null) return null;
    var lat = double.tryParse(response.body["latitude"]);
    var lng = double.tryParse(response.body["longitude"]);
    print("LATLNG $lat $lng");
    if(response.statusCode == 200 && lat != null && lng != null)
...
//update();
  ...
  void validateSlot(List<TimeSlotModel> slots, int dateIndex, {bool notify = true}) {
    ...
    for(int index=0; index<slots.length; index++) {
      print("${_day} and ${slots[index].day}");
      print(slots[index].endTime.isAfter(DateTime.now()) );
      print(slots[index].endTime );
      print("shhh ${(dateIndex == 0 ? slots[index].endTime.isAfter(DateTime.now()) : true)}");
      ...
      print("$index false");
        ...
        print("allllll ${_slotIndexList.length}");
    if(notify) {
      update();
    }
  }
