...
class _ForgetPassScreenState extends State<ForgetPassScreen> {
  ...
  void _forgetPass(String countryCode) async {
    ...
    // bool _isValid = false;
    // _numberWithCountryCode = await getInternationalPhoneNumber(countryCode,_phone);
    ...
    //   _isValid = false;
    // }
    /*if (_isValid && _email.isEmpty) {
      showCustomSnackBar('enter_phone_number'.tr +" "+ "or".tr +" "+"enter_email_address".tr);
    } else if (!_isValid) {
      showCustomSnackBar('invalid_phone_number'.tr);
    } else */
    if (_email.isNotEmpty && !GetUtils.isEmail(_email)) {
      showCustomSnackBar('enter_a_valid_email_address'.tr);
    } else {
      if(widget.fromSocialLogin) {
        widget.socialLogInBody.phone = _numberWithCountryCode;
        Get.find<AuthController>().registerWithSocialMedia(widget.socialLogInBody,false,false,false);
      }else {
        Get.find<AuthController>().forgetPassword(_email, _numberWithCountryCode).then((status) async {
          if (status.isSuccess) {
            Get.toNamed(RouteHelper.getVerificationRoute(_numberWithCountryCode, '', RouteHelper.forgotPassword, '',_email));
          }else {
