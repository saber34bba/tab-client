...
class _VerificationScreenState extends State<VerificationScreen> {
  ...
  void initState() {
    ...
    _number = widget.number.isEmpty ? "" : (widget.number.startsWith('+') ? widget.number : '+'+widget.number.substring(1, widget.number.length));
    _startTimer();
  }
          ...
          child: GetBuilder<AuthController>(builder: (authController) {
            return Column(children: [
              ...
              Get.find<SplashController>().configModel.demo ? Text(
                ...
                TextSpan(text:"\n"),
                if(_number.isNotEmpty)
                TextSpan(text: 'phone'.tr, style: robotoRegular.copyWith(color: Theme.of(context).disabledColor)),
                if(_number.isNotEmpty)
                TextSpan(text: ' $_number', style: robotoMedium.copyWith(color: Theme.of(context).textTheme.bodyText1.color)),
                if(widget.email.isNotEmpty)
                TextSpan(text:"\n"),
                if(widget.email.isNotEmpty)
                TextSpan(text: 'email'.tr, style: robotoRegular.copyWith(color: Theme.of(context).disabledColor)),
                if(widget.email.isNotEmpty)
                  ...
                  onPressed: _seconds < 1 ? () {
                    if(widget.fromSignUp) {
                      authController.login(_number, widget.password).then((value) {
                        ...
                        if (value.isSuccess) {
                          if(widget.email.isEmpty && "${value["email"]}".isNotEmpty){
                      ...
                      authController.forgetPassword(widget.email,_number).then((value) {
                        if (value.isSuccess) {
                          _startTimer();
                          showCustomSnackBar('resend_code_successful'.tr, isError: false);
                        } else {
              ...
              authController.verificationCode.length == 4 ? !authController.isLoading ? CustomButton(
                ...
                onPressed: () {
                  if(widget.fromSignUp) {
                    authController.verifyPhone(widget.email, _number, widget.token).then((value) {
                      if(value.isSuccess) {
                    ...
                    authController.verifyToken(widget.email, _number).then((value) {
                      if(value.isSuccess) {
