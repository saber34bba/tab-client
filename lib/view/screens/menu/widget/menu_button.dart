...
class MenuButton extends StatelessWidget {
  ...
  final bool isLogout;
  MenuButton({@required this.menu, bool isProfile = false, bool isLogout = false}):
    this.isProfile = isProfile ?? false,
    this.isLogout = isLogout ?? false;

  @override
  Widget build(BuildContext context) {
    int _count = ResponsiveHelper.isDesktop(context) ? 8 : ResponsiveHelper.isTab(context) ? 6 : 4;
    double _size = 54;//((context.width > Dimensions.WEB_MAX_WIDTH ? Dimensions.WEB_MAX_WIDTH : context.width)/_count)-Dimensions.PADDING_SIZE_DEFAULT;
      ...
      child: Padding(
        ...
        child: Row(children: [
          Container(
            ...
            // padding: EdgeInsets.all(Dimensions.PADDING_SIZE_DEFAULT/2),
            margin: EdgeInsets.symmetric(horizontal: Dimensions.PADDING_SIZE_SMALL/2),
            decoration: BoxDecoration(
              borderRadius: BorderRadius.circular(Dimensions.RADIUS_SMALL),
              color: isLogout ? ( Get.find<AuthController>().isLoggedIn() ? Colors.red : Colors.green ) : Colors.transparent,
              // boxShadow: [BoxShadow(color: Colors.grey[Get.isDarkMode ? 800 : 200], spreadRadius: 1, blurRadius: 5)],
            ),
            alignment: Alignment.center,
            child: isProfile ? ProfileImageWidget(size: _size) : Icon(menu.icon, size: _size * 0.6, color: isLogout ? Colors.white : Theme.of(context).primaryColor),
          ),
          SizedBox(height: Dimensions.PADDING_SIZE_EXTRA_SMALL),

          Text(menu.title, style: robotoMedium.copyWith(fontSize: _size * (isLogout ? 0.5 : 0.3)), textAlign: TextAlign.center),
